...
configure_scep_ca() {
    ...
    if [ ${#VALID_CERTS[@]} -lt 2 ]; then
        ...
        # NDES bundle-struktur (bekreftet):
        #   cert[0] = RA signing cert  (INADC01-MSCEP-RA)        → -r
        #   cert[1] = Root CA          (Indra Root CA)            → -R (CA-chain)
        #   cert[2] = RA encryption    (EnrollmentAgentOffline)   → -e
        #   cert[3] = Enterprise CA                               → -R (CA-chain)
        ...
        log_success "RA signing cert: $RA_SUBJECT"
        log "  Utløper: $RA_EXPIRES"

        # cert[2] = RA encryption (hvis finnes)
        if [ ${#VALID_CERTS[@]} -ge 3 ]; then
            cp "${VALID_CERTS[2]}" "$TMP_RA_ENC"
            RA_ENC_SUBJECT=$(openssl x509 -in "$TMP_RA_ENC" -noout -subject 2>/dev/null | sed 's/subject=//')
            log_success "RA encryption cert: $RA_ENC_SUBJECT"
        else
            log_warn "Ingen separat RA encryption cert funnet – bruker signing cert som fallback"
            cp "$TMP_RA" "$TMP_RA_ENC"
        fi

        # Kombiner signing + encryption i én fil → sendes som -r til certmonger
        # certmonger støtter ikke -e flagg, men håndterer begge RA-certs i én -r fil
        cat "$TMP_RA" "$TMP_RA_ENC" > "$TMP_RA_COMBINED"
        log_success "Kombinert RA-fil: $(grep -c 'BEGIN CERTIFICATE' "$TMP_RA_COMBINED") sertifikater → $TMP_RA_COMBINED"

        # Resten = CA-chain (cert[1] + cert[3] + eventuelle flere)
        > "$TMP_CA_CHAIN"
...
if [ "$TEST_MODE" = false ]; then
    ...
    echo "  - Splitte bundle: cert[0]=signing + cert[2]=encryption kombinert → -r, cert[1+3]=CA-chain → -R"
