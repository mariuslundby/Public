...
configure_scep_ca() {
    ...
    log_section "Steg 1: Last ned CA-bundle"
    rm -f "$TMP_BUNDLE" "$TMP_RA" "$TMP_CA_CHAIN" /tmp/ndes-cert-* 2>/dev/null

    log "Forsøker scep-submit -C ..."
    if timeout 30 "$SCEP_HELPER" -u "$SCEP_URL" -C "$TMP_BUNDLE" 2>&1 | tee -a "$LOG_FILE"; then
        if [ -s "$TMP_BUNDLE" ]; then
            CERT_COUNT=$(grep -c "BEGIN CERTIFICATE" "$TMP_BUNDLE" || echo 0)
            log_success "Lastet ned CA-bundle via scep-submit: $CERT_COUNT sertifikater"
        else
            log_warn "scep-submit returnerte tom fil"
            rm -f "$TMP_BUNDLE"
        fi
    else
        log_warn "scep-submit feilet"
        rm -f "$TMP_BUNDLE"
    fi
    ...
    log_section "Steg 2: Split RA-cert og CA-chain"
    ...
    if [ ${#VALID_CERTS[@]} -lt 2 ]; then
        ...
        CA_CERT_COUNT=$(grep -c "BEGIN CERTIFICATE" "$TMP_CA_CHAIN" || echo 0)
        log_success "CA-chain: $CA_CERT_COUNT sertifikater"
    fi

    # ── Steg 3: Installer i system trust store ───────────────────────────────

    log_section "Steg 3: Installer i system trust store"
    ...
    log_section "Steg 4: Legg til SCEP CA i certmonger"
...
if [ "$TEST_MODE" = true ]; then
    ...
    if [ -n "$ALL_REQUESTS" ]; then
        ...
        [ $CLEANED_COUNT -gt 0 ] && log_success "Cleaned up $CLEANED_COUNT old request(s)"
    fi
...
if [ "$TEST_MODE" = true ]; then
    ...
    if [ ! -f "$CA_CERT" ] || [ ! -s "$CA_CERT" ]; then
        ...
        if [ ! -s "$CA_CERT" ] && [ -s "$TMP_CA_CHAIN" ]; then
            log_warn "GetCACert feilet – bruker allerede nedlastet CA-chain"
...
if [ "$TEST_MODE" = true ]; then
    ...
    if [ -n "$ALL_WIFI_CONNECTIONS" ]; then
        ...
        [ $DEPRIORITIZED_COUNT -gt 0 ] && log_success "Deprioritized $DEPRIORITIZED_COUNT old profile(s)"
    fi
fi
