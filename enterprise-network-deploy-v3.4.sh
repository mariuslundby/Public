...
TMP_RA_ENC="/tmp/ndes-ra-enc.crt"      # RA encryption cert (cert[2])
TMP_RA_COMBINED="/tmp/ndes-ra-combined.crt" # RA signing + encryption kombinert → -r
...
configure_scep_ca() {
    ...
    if [ ${#VALID_CERTS[@]} -lt 2 ]; then
        log_warn "Færre enn 2 sertifikater – bruker hele bundle som -R og -r"
        cp "$TMP_BUNDLE" "$TMP_RA_COMBINED"
        cp "$TMP_BUNDLE" "$TMP_CA_CHAIN"
    else
        # v3.3: Innholdsbasert splitting – IKKE posisjonsbasert
        # Rekkefølgen i bundle varierer mellom scep-submit og GetCACert:
        #   scep-submit:  cert[0]=RA-signing, cert[1]=RootCA, cert[2]=RA-enc, cert[3]=EntCA
        #   GetCACert:    cert[0]=EntCA,       cert[1]=RootCA, cert[2]=RA-enc, cert[3]=RA-signing
        #
        # Identifisering via Basic Constraints:
        #   "Subject Type=CA" → CA-sertifikat → -R (CA-chain)
        #   Ingen CA Basic Constraints → RA-sertifikat → -r (kombinert)
        ...
        for cert_file in "${VALID_CERTS[@]}"; do
            ...
            # Sjekk om dette er et CA-sertifikat via Basic Constraints
            if echo "$CERT_TEXT" | grep -q "Subject Type=CA"; then
                cat "$cert_file" >> "$TMP_CA_CHAIN"
                CA_COUNT=$((CA_COUNT + 1))
                log "  CA-cert  → CA-chain: $SUBJECT"
            else
                # RA-sertifikat (CEPEncryption eller EnrollmentAgentOffline)
                KEY_USAGE=$(echo "$CERT_TEXT" | grep -A2 "Key Usage" | grep -v "Key Usage" | head -1 | xargs)
                TEMPLATE=$(echo "$CERT_TEXT" | grep "Certificate Template Name" -A1 | tail -1 | xargs)
                cat "$cert_file" >> "$TMP_RA_COMBINED"
                RA_COUNT=$((RA_COUNT + 1))
                log "  RA-cert  → combined -r: $SUBJECT [$TEMPLATE] [$KEY_USAGE]"
            fi
        done

        log_success "Splitting ferdig: $CA_COUNT CA-cert(s) → -R, $RA_COUNT RA-cert(s) → -r"

        if [ $RA_COUNT -eq 0 ]; then
            log_warn "Ingen RA-sertifikater funnet – bruker hele bundle som -r fallback"
            cp "$TMP_BUNDLE" "$TMP_RA_COMBINED"
        fi
        ...
        log_success "CA-chain: $CA_CERT_COUNT sertifikat(er), RA-combined: $RA_CERT_COUNT sertifikat(er)"
    fi
    ...
    # Metode 1: -R (CA-chain) + -r (kombinert RA signing+encryption)
    #           Bekreftet å fungere på Fedora 42 med certmonger 0.79.x
    #           Merk: -e flagget finnes ikke i denne versjonen av certmonger
    log "Metode 1: getcert add-scep-ca med -R og kombinert -r (signing+encryption)..."
    SCEP_OUT=$(getcert add-scep-ca         -c "$CA_NAME"         -u "$SCEP_URL"         -R "$TMP_CA_CHAIN"         -r "$TMP_RA_COMBINED" 2>&1)
    echo "$SCEP_OUT" | tee -a "$LOG_FILE"

    sleep 10

    if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
        log_success "CA '$CA_NAME' lagt til (metode 1: kombinert -r)"
    ...
    # Metode 2: -R og kun signing -r (Fedora 40 / eldre certmonger)
    log_warn "Metode 1 feilet – prøver metode 2 (-R og kun signing -r)..."
    getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
    sleep 3

    SCEP_OUT=$(getcert add-scep-ca         -c "$CA_NAME"         -u "$SCEP_URL"         -R "$TMP_CA_CHAIN"         -r "$TMP_RA" 2>&1)
    echo "$SCEP_OUT" | tee -a "$LOG_FILE"

    sleep 10

    if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
        log_success "CA '$CA_NAME' lagt til (metode 2: signing -r)"
        getcert list-cas -c "$CA_NAME" 2>&1 | tee -a "$LOG_FILE"
        record_state "CA_ADDED:$CA_NAME"
        sleep 5
        return 0
    ...
    # Metode 3: Hele bundle som -R og kombinert -r
    log_warn "Metode 2 feilet – prøver metode 3 (bundle -R, kombinert -r)..."
    getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
    sleep 3

    SCEP_OUT=$(getcert add-scep-ca         -c "$CA_NAME"         -u "$SCEP_URL"         -R "$TMP_BUNDLE"         -r "$TMP_RA_COMBINED" 2>&1)
    echo "$SCEP_OUT" | tee -a "$LOG_FILE"

    sleep 10

    if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
        log_success "CA '$CA_NAME' lagt til (metode 3: bundle -R kombinert -r)"
        getcert list-cas -c "$CA_NAME" 2>&1 | tee -a "$LOG_FILE"
        record_state "CA_ADDED:$CA_NAME"
        sleep 5
        return 0
    ...
    # Metode 4: Kun -R – siste fallback
    log_warn "Metode 3 feilet – prøver metode 4 (kun -R)..."
    getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
    sleep 3

    SCEP_OUT=$(getcert add-scep-ca         -c "$CA_NAME"         -u "$SCEP_URL"         -R "$TMP_BUNDLE" 2>&1)
    echo "$SCEP_OUT" | tee -a "$LOG_FILE"

    sleep 10

    if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
        log_success "CA '$CA_NAME' lagt til (metode 4: kun -R)"
        getcert list-cas -c "$CA_NAME" 2>&1 | tee -a "$LOG_FILE"
        record_state "CA_ADDED:$CA_NAME"
        sleep 5
        return 0
