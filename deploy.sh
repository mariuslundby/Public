...
# ─────────────────────────────────────────────
# SCEP CA konfigurasjon – v3.5.9 runtime flag-deteksjon
# ─────────────────────────────────────────────

configure_scep_ca() {
    log "Configuring SCEP CA: $CA_NAME (v3.5.9 – runtime flag-deteksjon, system TLS)"
    ...
    # ── v3.5.9 FIX: Runtime-test av scep-submit flagg ────────────────────────
    # Versjonsnummer er IKKE pålitelig – 0.79.x på Fedora 29 mangler -R/-N/-I
    # selv om minor >= 79. Test den faktiske binæren i stedet.
    local SCEP_SUPPORTS_R=false
    local SCEP_USAGE
    SCEP_USAGE=$("$SCEP_HELPER" --help 2>&1 || "$SCEP_HELPER" -? 2>&1 || true)
    if echo "$SCEP_USAGE" | grep -q -- '-R'; then
        SCEP_SUPPORTS_R=true
    fi
    log "scep-submit støtter -R flag (runtime-test): $SCEP_SUPPORTS_R"
...
if [ "$TEST_MODE" = false ]; then
    ...
    echo "  - Runtime-teste scep-submit flagg (ikke versjonsnummer)"
