...
# ─────────────────────────────────────────────
# SCEP CA konfigurasjon – v3.5.10 invocation-test flag-deteksjon
# ─────────────────────────────────────────────

configure_scep_ca() {
    log "Configuring SCEP CA: $CA_NAME (v3.5.10 – invocation-test flag-deteksjon, system TLS)"
    ...
    # ── v3.5.10 FIX: Invocation-test av scep-submit flagg ────────────────────
    # --help og versjonsnummer er IKKE pålitelig – 0.79.x på Fedora 29 viser
    # -R i help-output men krasjer med "Usage:" når certmonger faktisk kaller
    # scep-submit med -R for CSR-innsending.
    #
    # Definitiv test: kjør scep-submit med -R og en dummy-URL. Hvis den svarer
    # med "Usage:" betyr det at -R ikke støttes i submit-modus.
    # Hvis den prøver å koble til (og feiler med connection error), støttes -R.
    local SCEP_SUPPORTS_R=false
    local SCEP_TEST_OUTPUT
    SCEP_TEST_OUTPUT=$("$SCEP_HELPER" -u "https://localhost.invalid" -R /dev/null 2>&1 || true)
    if echo "$SCEP_TEST_OUTPUT" | grep -qi "Usage:"; then
        SCEP_SUPPORTS_R=false
        log "scep-submit invocation-test: -R ga 'Usage:' → IKKE støttet"
    else
        SCEP_SUPPORTS_R=true
        log "scep-submit invocation-test: -R akseptert (connection error = forventet)"
    fi
    ...
    if [ "$SCEP_SUPPORTS_R" = true ]; then
        ...
        # Gammel certmonger (Fedora 29, CentOS 7 etc): kun -u støttet
        # scep-submit bruker system CA bundle automatisk for TLS
        SCEP_HELPER_ARGS="-u $SCEP_URL"
        log_warn "Gammel scep-submit uten -R/-N/-I – bruker kun -u (system TLS)"
    fi

    log "scep-submit args: $SCEP_HELPER_ARGS"

    # Registrer CA via getcert
    # getcert add-scep-ca -R er et getcert-flagg (ikke scep-submit) og støttes bredt
    ...
    # Fallback: hvis getcert add-scep-ca heller ikke støtter -R (veldig gammel)
    if [ $GETCERT_ADD_EXIT -ne 0 ] || echo "$SCEP_OUT" | grep -qi "usage\|unknown option\|invalid"; then
        log_warn "getcert add-scep-ca -R feilet – prøver uten -R"
        SCEP_OUT=$(getcert add-scep-ca \
            -c "$CA_NAME" \
            -u "$SCEP_URL" 2>&1)
        echo "$SCEP_OUT" | tee -a "$LOG_FILE"
    ...
    # Oppdater certmonger intern config med korrekte helper-args
    ...
    # Skriv config med de flaggene scep-submit faktisk støtter
...
if [ "$TEST_MODE" = false ]; then
    ...
    echo "  - Invocation-teste scep-submit flagg (ikke --help eller versjon)"
    echo "  - Kalle getcert add-scep-ca med flagg som faktisk støttes"
