...
TMP_RA_ENC="/tmp/ndes-ra-enc.crt"   # RA encryption cert (cert[2]) – kreves av certmonger på Fedora 42+
...
configure_scep_ca() {
    ...
    rm -f "$TMP_BUNDLE" "$TMP_RA" "$TMP_RA_ENC" "$TMP_CA_CHAIN" /tmp/ndes-cert-* 2>/dev/null
    ...
    # Metode 1: Med -R (CA-chain), -r (RA signing) og -e (RA encryption)
    #           Kreves av certmonger på Fedora 42+ / nyere certmonger-bygg
    log "Metode 1: getcert add-scep-ca med -R, -r og -e flagg..."
    ...
    if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
        log_success "CA '$CA_NAME' lagt til (metode 1: -R -r -e)"
    ...
    # Metode 2: -R og -r uten -e (Fedora 40 / eldre certmonger)
    log_warn "Metode 1 feilet – prøver metode 2 (-R og -r, ingen -e)..."
    getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
    sleep 3
    ...
    if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
        log_success "CA '$CA_NAME' lagt til (metode 2: -R -r)"
        getcert list-cas -c "$CA_NAME" 2>&1 | tee -a "$LOG_FILE"
        record_state "CA_ADDED:$CA_NAME"
        sleep 5
        return 0
    ...
    # Metode 3: Hele bundle som -R og -r og -e
    log_warn "Metode 2 feilet – prøver metode 3 (bundle som -R, -r og -e)..."
    getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
    sleep 3
    ...
    if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
        log_success "CA '$CA_NAME' lagt til (metode 3: bundle -R -r -e)"
        getcert list-cas -c "$CA_NAME" 2>&1 | tee -a "$LOG_FILE"
        record_state "CA_ADDED:$CA_NAME"
        sleep 5
        return 0
    ...
    # Metode 4: Kun -R (hele bundle), siste fallback
    log_warn "Metode 3 feilet – prøver metode 4 (kun -R, hele bundle)..."
    getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
    sleep 3
...
if [ "$TEST_MODE" = false ]; then
    ...
    echo "  - Splitte bundle: cert[0]=RA signing (-r), cert[2]=RA encryption (-e), cert[1+3]=CA-chain (-R)"
